...
public class HabrCareerApiService {
    ...
    @Value("${app.habr.timeout:10000}")
    private int timeout;

    @Value("${app.habr.max-pages:10}")
    private int maxPages;

    @Value("${app.habr.enabled:false}")
    private boolean enabled;

    // Форматтер для дат Habr Career (поддерживает разные форматы)
    private final DateTimeFormatter sourceDateFormatter = new DateTimeFormatterBuilder()
        ...
        try {
            ...
            while (true) {
                ...
                if (currentPage >= maxPages) {
                    log.info("Habr Career pagination stopped at maxPages={}", maxPages);
                    break;
                }

                java.net.URI uri = buildSearchUri(request, currentPage);
                log.info("Habr Career search URL: {}", uri);

                ResponseEntity<Map> response = restTemplate.exchange(uri, HttpMethod.GET, entity, Map.class);
                Map<String, Object> body = response.getBody();
                if (body == null) {
                    log.warn("Habr Career response body is null for page {}", currentPage);
                    break;
                }
                ...
                log.info("Habr Career response: status={}, found={}, items={}, page={}, pages={}",
                ...
                if (items == null) {
                    log.warn("Habr Career response has no items. Keys={}", body.keySet());
                    break;
                }
                if (items.isEmpty()) {
                    if (found != null && found > 0) {
                        log.warn("Habr Career returned found={} but empty items array", found);
                    }
                    break;
                }
                ...
                if (!hasFresh) {
                    log.info("Habr Career pagination stopped: page {} is older than cutoff {}", currentPage, cutoff);
                    break;
                }
            ...
            log.error("Error searching vacancies on Habr Career: {}", e.getMessage(), e);
        }
        return new ArrayList<>();
    }
    ...
    private List<Vacancy> convertToVacancies(
        ...
        int errorCount = 0;
        int skippedCount = 0;
        int skippedByTitleCount = 0;

        for (Map<String, Object> item : items) {
            try {
                String title = item.get("name") instanceof String ? (String) item.get("name") : null;
                ...
                if (publishedAt != null) {
                    try {
                        LocalDateTime publishedDateTime = LocalDateTime.parse(publishedAt, sourceDateFormatter);
        ...
        log.info("Successfully converted {}/{} vacancies from Habr Career response", successCount, items.size());
        if (skippedCount > 0) {
            log.info("Skipped {} vacancies older than cutoff", skippedCount);
        }
        if (skippedByTitleCount > 0) {
            log.info("Skipped {} Habr vacancies because title does not match query '{}'", skippedByTitleCount, query);
        }
        if (errorCount > 0) {
